function Game8910 () {
    ...
    this.initUI = function () {
        ...
        $( '.mainPlayer1' ).off( 'click' ).on( 'click', function (event) {
            self.handleBtnEntry( event, $( this ).data( 'player' ), false );
        } );
        $( '.mainPlayer2' ).off( 'click' ).on( 'click', function (event) {
            self.handleBtnEntry( event, $( this ).data( 'player' ), false );
        } );
        $( '.mainPlayer1' ).off( 'taphold' ).on( 'taphold', function (event) {
            self.handleBtnEntry( event, $( this ).data( 'player' ), true );
        } );
        $( '.mainPlayer2' ).off( 'taphold' ).on( 'taphold', function (event) {
            self.handleBtnEntry( event, $( this ).data( 'player' ), true );
        } );
        $( '.foulWrapper' ).off( 'tap' ).on( 'tap', function (event) {
            self.handleFoulClick( event, $( this ).data( 'player' ) );
        } );
